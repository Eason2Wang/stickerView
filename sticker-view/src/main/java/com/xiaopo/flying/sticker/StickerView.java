...
public class StickerView extends FrameLayout {
  ...
  public boolean isStickerActive;
  ...
  public void zoomSticker(Sticker sticker, @NonNull float factor, PointF position) {
    float offsetX = position.x - sticker.getWidth() / 2;
    float offsetY = position.y - sticker.getHeight() / 2;
    sticker.getMatrix().setTranslate(offsetX, offsetY);
    sticker.getMatrix().postScale(factor, factor);
    invalidate();
  }
  ...
  protected void addStickerImmediately(@NonNull Sticker sticker, PointF position) {
    setStickerPosition(sticker, position);
  ...
  protected void setStickerPosition(@NonNull Sticker sticker, PointF position) {
    float offsetX = position.x - sticker.getWidth() / 2;
    float offsetY = position.y - sticker.getHeight() / 2;
    sticker.getMatrix().postTranslate(offsetX, offsetY);
  }
